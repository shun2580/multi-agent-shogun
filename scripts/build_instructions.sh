...
build_instruction_file() {
    ...
    if [ -f "$original_file" ]; then
        awk '{ sub(/\r$/, "") } /^---$/{if(++n==2) {print "---"; exit} if(n==1) next} n==1' "$original_file" > "$output_path"
        echo "" >> "$output_path"
    else
...
EOFYAML
    ...
    if [[ "$cli_type" == "opencode" ]]; then
        # Phase3 (cmd_063): opencode reads the handwritten body directly,
        # matching how Claude itself reads instructions/{role}.md.
        # CRLF-safe: strip \r before matching, only treat the FIRST TWO
        # "---" lines as front-matter delimiters (n<2 guard) so that any
        # later "---" horizontal rule in the body is preserved as literal
        # content instead of being swallowed.
        awk '{ sub(/\r$/, "") } /^---$/ && n<2 { n++; next } n>=2 { print }' "$original_file" >> "$output_path"
    else
...
mode: primary
# Auto-generated by build_instructions.sh — do not edit manually.
# Source: instructions/${role}.md (handwritten body) + instructions/cli_specific/opencode_tools.md
...
FRONTMATTER
        ...
        {
            awk '{ sub(/\r$/, "") } /^---$/ && n<2 { n++; next } n>=2 { print }' "$PARTS_DIR/${role}.md"
