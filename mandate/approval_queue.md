...
- ID: AQ-018 | 日付: 2026-09-16 | 操作内容: pretooluse_reversibility_check.sh
  ...
  補遺§(c)条件式(cmd_194 工程6'、殿裁定 fable_ruling_20260916_cmd194_step6.md
  §(c)、subtask_194_step6prime起票): 「補遺§(c)条件式: (a)(b)適用後、
  切り詰め無しのWOULD-BLOCKが20件以上・セッション2回以上蓄積し、軍師の
  全件再判定で偽陽性0件。充足時にenforceモードの実装を起票し、実装後に
  原則4で移行を裁可する。未充足の間はobserveを維持する(『観測専用のまま
  恒久維持』とは明文化しない——状態を持たせることで帳簿の乖離を防ぐという
  裁定の趣旨による)。」機械判定可能な観測点は`mandate/verifiers.md`
  「条件式(c)」節を参照。

  過去の125/49件による条件3判定の無効化(逐語): 「(a)(b)適用前の再判定
  不能33件(および工程6時点の32件)は、偽陽性にも真陽性にも数えない。
  過去の125件(または48/49件)のWOULD-BLOCK実測による条件3判定は無効と
