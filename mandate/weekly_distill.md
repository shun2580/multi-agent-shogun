...
**本スクリプト・cron設定は初回実行を保留としている。**

理由: decisions_journal.mdが十分なボリュームに達するまで実行すると、
偽陽性の原則化(実運用で実証されていない仮説)や過度な粒度の原則追加を招く。

**初回実行は、殿が明示指示するまで延期する。** 

- 保留中でも cron/スケジュール設定は設置済み。殿からの指示があれば
  すぐに実行可能な状態を保つ。
- 初回実行開始時は、このファイルの「初回実行について」節を削除し、
  殿の指示内容(決定日時)を記録すること。
...
**注意: 初回実行は保留。以下の設定は用意済みだが、殿からの実行指示まで起動しない。**
